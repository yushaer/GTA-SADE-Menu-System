 ...
 */
	...
	generate_scroll(){
		if(this.#options.length>this.#options_per_page){
			this.#scrollabe=true;
			this.#scroll=this.#options_per_page;
	...
	calculate_height(){
		if(this.#options.length<=this.#options_per_page){
			...
			this.#height=this.#options_per_page*(this.#outline_height+this.#heightoffset)+this.#heightoffset+10;
		}
	}
/**
 ...
 */
	...
	/**
	 * Sets the key binding to go up or down the menu
	 * @param {int} up - KeyCode for up key 
	 * @param {int} down - KeyCode for down key 
	 */
	setControls(up,down){
		this.#controls.up=up;
 ...
 */
	...
	/**
	 * sets the text color of the menu option
	 * @param {Array<int>} color - A int array of length 4 containing the rgba value for the color 
	 */
	...
	/**
	 * Sets the color of menu title text
	 * @param {Array<int>} color - A int array of length 4 containing the rgba value for the color 
	 */
...
/**
 * Sets number of menu options in view at a time 
 * @param {int} item_count - A int value representing the max menu options in view 
 */
	setOptionsPerPage(value){
		this.#options_per_page=value;
		this.generate_scroll();
 ...
 */
	...
	update(){
		...
		this.drawText(this.#name,this.#posx+(this.#width/2),this.#posy+2,this.#titlefont,this.#title_font_scale,true,this.#titlecolor,true);
	...
	draw(){
		...
		log("buttonpos" + Pad.GetPositionOfAnalogueSticks(1).leftStickY)
		...
		if(Pad.IsKeyPressed(this.#controls.up)){
			...
			if(this.#selected_index<0){
				...
				if(this.#scrollabe){
					this.#start_idx=(this.#options.length)-this.#options_per_page ;
